...
const make = (over: { accounts?: any[]; registerThrows?: boolean } = {}) => {
  ...
  return { service: new PaymentsService(prisma, audit, registers), prisma, registers };
};

describe('PaymentsService.pay (validações pré-transação)', () => {
  it('rejeita quando o operador não tem caixa aberto (409)', async () => {
    const { service } = make({ registerThrows: true });
    await expect(service.pay(['a1'], [{ method: 'CASH' as any, amount: '42.65' }], 'u1')).rejects.toBeInstanceOf(ConflictException);
  });
  ...
  it('rejeita conta inexistente/não OPEN (409)', async () => {
    ...
    await expect(service.pay(['a1'], [{ method: 'CASH' as any, amount: '10.00' }], 'u1')).rejects.toBeInstanceOf(ConflictException);
  });

  it('rejeita soma de tenders ≠ total (RB-037 → 400)', async () => {
    const { service } = make({ accounts: [accountOpen('a1', '42.65')] });
    await expect(service.pay(['a1'], [{ method: 'CASH' as any, amount: '40.00' }], 'u1')).rejects.toBeInstanceOf(BadRequestException);
  });
});
